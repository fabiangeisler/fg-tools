...
def createRuntimeCommand(commandName, command, annotation='', category='', commandLanguage='python', default=True):
    '''
    Creates a custom command in Maya that can be accessed via Mel or Python at runtime. Be aware that you can not change an already created
    runtime command. You have to restart Maya and create it again.
    ...
    :param str category: (optional) You can place this command in a certain category so it's more logally sorted in the hotkey editor.
                         You can also have subcategories by inserting a "." between them (i.e. "MainCategory.SubCategory")
    ...
    if not pymel.core.runTimeCommand(commandName, exists=True):
        ...
        pymel.core.warning(('The runtime command "{0:s}" already exists and can not be overwritten. '
                            'To change an established runtime command you need to restart maya and create it again.'.format(commandName)))
